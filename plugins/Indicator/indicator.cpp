...
void Indicator::install() {
    ...
    m_installProcess.start("bash /opt/click.ubuntu.com/indicator-immersive.kugiigi/current/indicator/install.sh");
}

void Indicator::uninstall() {
    //TODO don't hardcode this
    m_uninstallProcess.start("bash /opt/click.ubuntu.com/indicator-immersive.kugiigi/current/indicator/uninstall.sh");
}
